...
bool FileManager::is_file_open(const std::string& tag)
{  
    // checks if a file is currently open - associated with a tag
    return files_.count(tag) > 0;
}

bool FileManager::delete_file(const std::string& tag)
...
std::string FileManager::calculate_checksum(const std::string& tag)
...
bool FileManager::compare_files(const std::string& tag1, const std::string& tag2)
...
int FileManager::get_file_size(const std::string& tag)
...
std::string FileManager::get_file_path(const std::string& tag)
...
std::string FileManager::find_tag_by_path(const std::string& file_path)
...
int FileManager::get_last_modified_time(const std::string& tag)
...
std::string FileManager::get_last_modified_date(const std::string& tag)
...
std::string FileManager::generate_tag(const std::string& file_path)
...
bool FileManager::create_empty_file(const std::string& path)
{
    // creates empty file, does not keep it open for further access
...
js::json FileManager::get_json_contents(const std::string& path)
{
    ...
    if(!file.is_open())
    {
        throw std::runtime_error(ERROR_READING_FILE);
    }
